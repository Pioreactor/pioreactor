...
if [ "$HOSTNAME" = "$LEADER_HOSTNAME" ]; then
    install_checked_asset \
        "$SCRIPT_DIR/03_alt_media_fraction.yaml" \
        "$DOT_PIOREACTOR/ui/charts/03_alt_media_fraction.yaml"
    install_checked_asset \
        "$SCRIPT_DIR/08_current_volume_ml.yaml" \
        "$DOT_PIOREACTOR/ui/charts/08_current_volume_ml.yaml"
    install_checked_asset \
