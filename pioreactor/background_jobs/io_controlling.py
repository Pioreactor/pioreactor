...
class IOAlgorithm(BackgroundSubJob):
    ...
    def execute_io_action(self, alt_media_ml=0, media_ml=0, waste_ml=0, log=True):
        ...
        if alt_media_ml > max_:
            ...
            self.execute_io_action(
                alt_media_ml=0, media_ml=media_ml / 2, waste_ml=media_ml / 2, log=False
            )
            self.execute_io_action(
                alt_media_ml=alt_media_ml,
                media_ml=media_ml / 2,
...
class AlgoController(BackgroundJob):
    ...
    def __init__(self, io_algorithm, unit=None, experiment=None, **kwargs):
        ...
        self.check_for_existing_io_controlling_process()
    ...
    def check_for_existing_io_controlling_process(self):
        ...
        if sum([p == "io_controlling" for p in pio_jobs_running()]) > 0:
