...
biomass_signal=$(/opt/pioreactor/venv/bin/crudini --get "$CONFIG" dosing_automation.turbidostat biomass_signal 2>/dev/null || true)
if [ -z "$biomass_signal" ]; then
    biomass_signal=$(/opt/pioreactor/venv/bin/crudini --get "$CONFIG" turbidostat.config biomass_signal 2>/dev/null || true)
fi
if [ -z "$biomass_signal" ]; then
    biomass_signal="auto"
fi
/opt/pioreactor/venv/bin/crudini --set "$CONFIG" dosing_automation.turbidostat biomass_signal "$biomass_signal"
