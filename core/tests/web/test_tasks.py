...
def test_periodic_camera_capture_captures_when_snapshot_is_due(
    ...
    monkeypatch.setattr(tasks, "camera_snapshot_is_due", lambda unit, experiment, interval: True)
    monkeypatch.setattr(tasks, "is_pio_job_running", lambda job_name: False)
    monkeypatch.setattr(
        tasks,
        "subscribe",
        lambda *_args, **_kwargs: pytest.fail("camera must not wait when OD reading is not running"),
    )
...
def test_periodic_camera_capture_waits_for_live_od_reading(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_lock("camera-lock")
    events: list[str] = []
    ...
    monkeypatch.setattr(tasks, "camera_snapshot_is_due", lambda unit, experiment, interval: True)
    monkeypatch.setattr(tasks, "is_pio_job_running", lambda job_name: True)

    def observe_next_od_reading(
        topic: str,
        *,
        timeout: float,
        allow_retained: bool,
        ...
        assert topic == "pioreactor/unit-a/experiment-a/od_reading/ods"
        assert timeout == tasks.CAMERA_OD_ALIGNMENT_TIMEOUT_SECONDS
        assert allow_retained is False
        assert name == "scheduled-camera"
        events.append("od_reading")
        return object()

    def capture_after_od_reading(
        unit: str, *, experiment: str | None, capture_reason: str
    ) -> CameraStillMetadata:
        events.append("camera")
        return metadata

    monkeypatch.setattr(tasks, "subscribe", observe_next_od_reading)
    monkeypatch.setattr(tasks, "capture_camera_still", capture_after_od_reading)

    result = tasks.capture_camera_still_periodic_task.call_local()

    assert result["captured"] is True
    assert events == ["od_reading", "camera"]


def test_periodic_camera_capture_skips_when_od_alignment_times_out(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _clear_lock("camera-lock")
    monkeypatch.setattr(tasks, "camera_is_enabled", lambda: True)
    ...
    monkeypatch.setattr(tasks, "camera_snapshot_is_due", lambda unit, experiment, interval: True)
    monkeypatch.setattr(tasks, "is_pio_job_running", lambda job_name: True)
    monkeypatch.setattr(tasks, "subscribe", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        tasks,
        "capture_camera_still",
    ...
    assert result == {"captured": False, "reason": "od_alignment_timeout"}
