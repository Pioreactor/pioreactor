...
# --- environment --------------------------------------------------------------
$(VENV_DIR)/bin/activate: requirements/bootstrap.txt ## Create virtual env + core tooling
...
api-test: venv  ## API (Flask) tests only
...
# --- live dev servers ---------------------------------------------------------
api-dev: venv  ## Run Flask API on 127.0.0.1:5000
    @FLASK_ENV=development $(ACTIVATE) && cd $(API_DIR) && python3 -m flask  --app main run -p 4999 --debug

frontend-dev: frontend-install ## Run React dev server on :3000
