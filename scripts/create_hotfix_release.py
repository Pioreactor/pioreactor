...
# -*- coding: utf-8 -*-
"""
Automate the production hotfix release flow described in the ops runbook.
...
Usage examples:
  python3 scripts/create_hotfix_release.py                  # release the current YY.M.N candidate from current branch
  python3 scripts/create_hotfix_release.py --series 26.3    # release in an explicit YY.M series
  python3 scripts/create_hotfix_release.py --dry-run        # print actions only
  python3 scripts/create_hotfix_release.py --force          # skip branch and dirty working tree guards
...
def ensure_valid_hotfix_source_branch(branch: str) -> None:
    if branch == "HEAD":
        raise RuntimeError("Detached HEAD is not supported. Check out a hotfix branch first.")
    if branch in PROTECTED_BRANCHES:
        protected = ", ".join(sorted(PROTECTED_BRANCHES))
        raise RuntimeError(
            f"Current branch is '{branch}', but hotfix releases must start from a non-protected branch. "
...
def ensure_update_sql_has_busy_timeout(dry_run: bool) -> bool:
    ...
    if update_sql_path.exists():
        ...
        current = ""
        print(f"Creating {update_sql_path} with update SQL footer")

    if dry_run:
        print(f"DRY-RUN: would write {update_sql_path}")
        return True
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Automate production hotfix release git workflow.")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if not args.force:
            ensure_valid_hotfix_source_branch(current_branch)
        ...
        print(f"Preparing production hotfix release for {release_version} from {current_branch}\n")
