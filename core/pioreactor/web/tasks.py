...
def pio_run(
    *args: str,
    ...
    grace_s: float = 1.00,  # how long to watch for "fast-fail"
) -> dict[str, Any]:
...
def estimator_save_estimator(device: str, estimator_payload: dict[str, object]) -> dict[str, str]:
    ...
    all_estimators = subclass_union(EstimatorBase)
    estimator = json_decode(json_encode(estimator_payload), type=all_estimators)
...
def repair_system() -> dict[str, Any]:
    ...
    if status_result.returncode == 0 and repair_result.returncode == 0:
        logger.debug(
            "Repair task finished successfully. %s",
            (status_result.stdout or "").strip()[:2000],
        )
    return {
