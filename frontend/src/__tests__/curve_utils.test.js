...
describe("curve_utils", () => {
  ...
  test("evaluateCurve selects spline or polynomial", () => {
    ...
    expect(evaluateCurve(0.25, "spline", splineData)).toBeCloseTo(1.25);
    expect(evaluateCurve(2, "poly", { coefficients: [1, 0] })).toBeCloseTo(2);
  });

  test("generateCurveData falls back to default domain", () => {
    const calibration = {
      curve_type: "poly",
      curve_data_: { coefficients: [1, 0] },
      recorded_data: { x: [], y: [] },
    };
